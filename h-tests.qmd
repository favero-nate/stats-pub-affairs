...
## Chapter Case Study: Ideology of Representatives in the U.S. Congress {.unnumbered}

We learned in @sec-qual-associations that we can use a comparison of means to evaluate how a binary (qualitative) variable relates to a quantitative variable. To see how hypothesis testing applies when comparing two means, we will use data describing U.S. congressional districts. Each district elects a single member (a Representative) to the U.S. House of Representatives. There is a binary variable `rural` indicating whether a majority of the district’s residents live in a rural area. A quantitative variable `ideology` is an estimate of political ideology for the Representative from that district.[^h-tests-1] Ideology can be measured on a continuous left-to-right scale (also called “liberal” to “conservative” in the US political system), with higher values indicating an official is further to the right.

With hypotheses testing, we evaluate a hypothesis called a **null hypothesis**. More details will be provided in the main part of the chapter, but normally the null hypothesis claims that there is no association between the independent and dependent variables. In the context of a regression, a coefficient of 0 indicates no association, so if we are using $\beta$ to represent the regression coefficient of interest, we can write the null hypothesis to be tested as $\beta = 0$.

The Regression Output presents results for a regression where ideology is the dependent variable and the binary variable `rural` is the independent variable. The table includes confidence intervals for the coefficients:

```         
Regression Output (ideology is the dependent variable)
...
In this case, assessing substantive size is a bit difficult because ideology is measured in unfamiliar units. Ideology scores (among Representatives in this sample) range from -4.10 to 4.57, and the standard deviation is 2.85. Since the smallest number (in absolute value) within the confidence interval for the `rural` coefficient is its lower bound of 1.42, we can say that the systematic difference between urban and rural districts appears to be at least half a standard deviation ($1.42 \div 2.85 = 0.498$). A difference of half a standard deviation is generally considered to be quite large, so we can say that Representatives from rural districts are notably further to the right politically (more conservative) than Representatives from urban districts.
...
Every null hypothesis should be paired with an **alternative hypothesis**, which is the logical opposite of the null hypothesis. We write $H_A$ (or $H_1$) to represent the alternative hypothesis, and our alternative to @eq-rural-null-equal-means is:
...
Moving forward, we will focus on the final form in which we've written the null and alternative hypotheses (@eq-rural-null-beta and @eq-rural-alt-beta). We already saw an example in the chapter case study of a null hypothesis being evaluated using a confidence interval. In that example, we reject the null hypothesis, and therefore we also *accept* the alternative hypothesis. Given that null and alternative hypotheses are logical opposites, we accept the alternative hypothesis whenever we reject the null hypothesis.

## Testing Relationships for Various Types of Variables
...
We already saw in our chapter case study how to evaluate this when the qualitative variable is binary, but what about when it is not? Consider the qualitative variable `region`, which divides the country into four different parts (using U.S. Census Bureau regions).[^h-tests-3]

[^h-tests-3]: <https://www2.census.gov/geo/pdfs/maps-data/maps/reference/us_regdiv.pdf>

To get an initial sense of how `region` is related to `ideology`, we can examine the box plots in @fig-ideology-region. On average, the Representatives from the Northeast and West appear to be markedly more left-leaning (liberal) than Representatives from the Midwest and South.

In the following discussion (as in the rest of this chapter), we will focus mainly on interpreting results from regression tables, and there will be a lot of details to keep track of. But remember, you can always refer back to @fig-ideology-region if you get lost. Note, however, that a simple plot of box plots does not allow you to directly evaluate hypothesis tests. One reason why is that we don't learn anything about sample size from box plots, and the ability to find statistical significance partially depends on sample size. For example, two distributions that look very different in a box plot may fail to have a statistically significant difference if the sample size of one or both distributions is very small. Still, a visualization like @fig-ideology-region can help us think about the big picture of what we are seeing from two variables. Having this big picture in mind can help us avoid mistakes while we are down "in the weeds" of evaluating regression coefficients.

![Box plots of the political ideology of U.S. Representatives, by region](Images/h-tests/ideology_region.png){#fig-ideology-region width="450"}

@tbl-ideology-region shows results for a regression where the independent variable is `region`. Northeast is the omitted category (see @sec-prediction-with-multiple-categories).
...
Though a simple regression slope coefficient and correlation coefficient normally have different values, their signs are always the same, as noted in @sec-correlation. Tests of the above null hypotheses (@eq-ideology-null-beta and @eq-ideology-null-rho) also yield equivalent p-values, so we will obtain equivalent results regardless of which statistic ($\hat{\beta}$ or $\hat{\rho}$) we use for our hypothesis testing. We will focus here on using regression.
...
In our example, the 90% confidence interval for the rural coefficient is \[1.559, 3.029\]. The 90% confidence interval is always a bit narrower than the 95% one (as seen if we compare to the regression output in the chapter case study). In this case, we reject the null hypothesis (@eq-rural-null-beta-1-sided) because the entire confidence interval consists of positive (non-zero) values. We therefore accept the alternative hypothesis.
...
-   When looking at regression results: If coefficient A is statistically significant and coefficient B is not, that doesn’t necessarily mean variable A matters more (to the prediction). For example, coefficient B could simply have a wider confidence interval, making it harder to draw firm conclusions about it.[^h-tests-7]
