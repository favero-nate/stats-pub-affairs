...
\[***Working draft notes for instructors**: I ended up deciding to mostly keep the examples for now, but some have been revised/restructured for clarity and consistency.*\]
...
Where do ordinal variables fit into this discussion? For many types of analysis, ordinal variables can be treated as either qualitative (for maximum flexibility) or as quantitative (if we are willing to assign fixed numbers to the various values, as we saw in the first example in @sec-critically-evaluating-graphs). Often, analysis will be simpler if we treat ordinal variables as quantitative, so that can be a good place to start. You can always then follow up with an analysis treating the ordinal variable as qualitative, which allows you to check if the results indicate something similar to what you saw when you treated the variable as quantitative.

::: {.callout-tip icon="false"}
## Chapter Case Study: Gender Differences in Extraversion {.unnumbered}

Let’s say I’m interested in studying how personality relates to gender. The most common model of personality in psychology is called the “Big Five." One common measure consists of 50 survey items (the IPIP Big-Five Factor Markers), with 10 items for each of the five personality traits from the model.[^qual-associations-1] @fig-Big5 shows some of these questions and how they are formatted.

![Example survey items used in the "Big Five" case study.](Images/qual-associations/Big5.jpg){#fig-Big5 width="450"}

For now, I decide to focus on whether people are introverted or extraverted. Extraverts are outgoing and tend to enjoy interacting with others. Extraverts will tend to agree with the statement “I am the life of the party” while introverts will tend to agree with the item “I don’t talk a lot.”

I find a dataset that contains many responses to the Big Five personality questions as well as information on the gender of each respondent.[^qual-associations-2] The gender variable was measured by asking respondents “What is your gender?”, and those selecting male were coded as 1s while females were coded as 2s.[^qual-associations-3] There are 10 different questions related to extraversion, and the dataset has one variable for each of these 10 questions (@fig-big5dataset). The variable labeled e1 shows responses to the item “I am the life of the party.” A value of 1 means the respondent disagrees with this statement, while a 3 indicates neutral, and a 5 means they agree.

![Screenshot of some observations and variable values in the "Big Five" dataset.](Images/qual-associations/big5dataset.jpg){#fig-big5dataset width="450"}

For all of the odd-numbered extraversion questions (e1, e3, e5, etc.), agreement indicates extraversion. For the even-numbered items (e2, e4, e6, etc.), agreement indicates introversion. To create a single extraversion variable that combines responses from all 10 survey items, I create a tally, adding up all the values for odd-numbered questions and then subtracting the responses to the even-numbered questions. An extreme extravert will have a 5 for all the odd-numbered questions and a 1 for all of the even-numbered ones, giving them a score of 20 ($5 \times 5-5 \times 1=20$). An extreme introvert will have a -20 since they will answer 1 to all the odd-numbered questions and 5 to all the even-numbered ones ($5 \times 1-5 \times 5=-20$).

As @fig-ExtHist indicates, most people lie somewhere in the middle between introversion and extraversion.

![Distribution of the extraversion trait.](Images/qual-associations/hist-extraversion.png){#fig-ExtHist width="400"}

If I want to describe differences in extraversion by gender in this dataset, I can divide my sample into two subsamples: males and females. I can then compare the distributions from these subsamples, using the tools to describe distributions that we learned in @sec-describing-one-variable. For example, I can construct two box plots side-by-side, as shown in @fig-GenderExtHist. The box and median line are a bit higher up for females, indicating that females are a bit more extraverted on average. But of course, there is abundant variation in extraversion among both groups: there are plenty of females who are introverts and plenty of males who are extraverts.

![Distribution of extraversion by female/male identity.](Images/qual-associations/boxplot-extraversion-gender-binary.png){#fig-GenderExtHist width="400"}
:::

[^qual-associations-1]: Goldberg, L. R. (1992). The development of markers for the Big-Five factor structure. *Psychological assessment*, *4*(1), 26.

[^qual-associations-2]: <https://openpsychometrics.org/_rawdata/> (the file I used is called “BIG5.zip”)

[^qual-associations-3]: Respondents were also given the option to select "other" (coded as 3), and some respondents skipped this question and were coded as 0.

## Describing the Relationship between Two Qualitative Variables {#sec-describing-the-relationship-between-two-qualitative-variables}

The relationship between two qualitative variables can be examined using a contingency table (also known as "crosstabs"). For example, @tbl-diet-row-percentages shows the data from the Mediterranean Diet and Health case study,[^qual-associations-4] in which 605 survivors of a heart attack were randomly assigned to follow either (1) a low-fat diet similar to one recommended at the time by the American Heart Association (AHA) or (2) a Mediterranean-type diet rich in vegetables, fruits, and grains. The percentages shown in this table are calculated by row, meaning that they are found by dividing the frequency in each cell by the total for that row, shown in the final column.

[^qual-associations-4]: <https://onlinestatbook.com/2/case_studies/diet.html>
...
We can reach an equivalent conclusion by examining percentages that are calculated by column, as shown in @tbl-diet-column-percentages. Here, percentages are calculated by dividing the frequency in each cell by the total shown at the bottom of the column. Sometimes, tables instead indicate the percent in the cell compared to the whole table. But calculating percentages by rows or by columns is usually the most helpful in the context of statistical analysis, so that is what we focus on here.
...
### Visualizing a Qualitative-Qualitative Relationship[^qual-associations-5]

[^qual-associations-5]: The last two paragraphs of this subsection are adapted from David M. Lane. “Graphing Qualitative Variables.” *Online Statistics Education: A Multimedia Course of Study*. [https://onlinestatbook.com/2/graphing_distributions/graphing_qualitative.html](#0){.uri}

Let us now visualize the relationship we have just discussed based on contingency tables. Bar charts are often an effective choice, although there are many different ways that such charts can be configured, and it may take some trial and error to find the most effective chart for any given data. In this case, because the number of individuals for each of the two diets is roughly the same (303 for the AHA diet and 302 for the Mediterranean diet), it makes it fairly easy to create a helpful chart. We clearly see from the clustered bar chart in @fig-diet-health that more people on the Mediterranean diet have the healthy outcome, while the various negative outcomes are all more common among people on the AHA diet.

![A bar chart of health outcomes by diet](Images/qual-associations/barchart-diets.png){#fig-diet-health width="400"}

We can also depict this data using a stacked bar chart to directly plot the percentages (rather than the counts) from a contingency table, as in @fig-stacked-barchart-diets. As noted above in our discussion of contingency tables, comparing percentages is an effective way to evaluate relationships even when a sample is not evenly distributed across categories (e.g., if there were many more people on the AHA than the Mediterranean diet). Thus, the stacked bar chart displaying percentages is more versatile in helping clarify associations among qualitative variables. At the same time, this kind of graph leaves out information about the underlying counts: we cannot tell from @fig-stacked-barchart-diets that most people in the sample have the healthy outcome.

![A stacked bar chart of health outcomes by diet](Images/qual-associations/stackedbarchart-diets.png){#fig-stacked-barchart-diets width="350"}
...
## Describing the Relationship between a Qualitative and a Quantitative Variable

When we have a quantitative variable and a qualitative variable, we typically begin by dividing our sample into sub-samples according to the qualitative variable. As we saw in the chapter case study, if we are examining the qualitative variable gender (indicating male or female), this means dividing the sample into males and females. We can then examine measures of central tendency for the sub-samples and compare them. This is exactly what we did when we compared the median lines in the box plots describing the distribution of the extraversion variable among females and males. We also saw an example of evaluating a qualitative-quantitative relationship in @sec-critically-evaluating-graphs, where we compared the means of the support-for-innovation variable among males versus females. Typically, we compare means, but we can also compare medians if we are concerned about our results being sensitive to outliers. Ultimately, we want to observe whether the means/medians appear similar across groups or if some groups have meaningfully higher means/medians than others. In the example with sex and support for innovation, the means were nearly the same (3.787 for females, 3.798 for males), suggesting that the two variables are not related (at least not to a meaningful extent). When it came to extraversion, a more meaningful difference between females and males was apparent. (Note that we used boxplots to compare the two subsamples when examining extraversion, and since boxplots require drawing axes that show the entire range of the data, a very tiny difference relative to the range of the quantitative variables would not be visually noticeable in a graph showing boxplots.)
...
Although bar charts can display means, we do not generally recommend them for this purpose. Box plots should generally be used instead since they provide more information than bar charts without taking up more space. For example, a box plot of the mouse-movement data is shown in @fig-boxplotmousemovement. You can see that @fig-boxplotmousemovement reveals more about the distribution of movement times than does @fig-barchartmeantime. While use of box plots is a good general rule, quantitative variables that take on only a few discrete values (such as the number of children someone has) are not very suitable for box plots, as noted in @sec-describing-one-variable. For such variables, it is better to stick with displaying means in a bar chart (or something similar like @fig-gender-compare-zoomed-out).
...
### Regression with a Qualitative Independent Variable {#sec-regression-with-a-qualitative-independent-variable}

Regression analysis is a very flexible set of tools that can be used to examine almost any sort of relationship among variables. For now, we will focus on understanding how it can be used to describe the relationship between a *quantitative* dependent variable and a *qualitative* independent variable. If you want to explore other possibilities, the chapter appendix shows how you can use regression when the dependent variable is qualitative.

#### Predicting extraversion using gender

Let us revisit the chapter case study to see how regression can be used to predict extraversion based on gender. If I want to describe differences in extraversion by gender in this dataset, I can compute the mean value of extraversion for males and for females. It turns out that males have an average extraversion of -0.46 while females’ average level of extraversion is 0.53. Thus, the average female is about 1-point more extraverted than the average male.
...
$$ \widehat{Extraversion}=0.53 \times Female-0.46 \times Male $$ {#eq-ExtReg}

Notice I’ve added a “hat” above the name of the variable *Extraversion*; this hat means that I’m making a guess about the value of that variable (I’m guessing the level of extraversion based on gender). The equation has two other indicator variables *Female* and *Male*, and these two variables will take on a value of 1 if the person’s gender is equal to the name of the variable and will otherwise take on a value of 0. For a female, *Female* will equal 1 and *Male* will equal 0, giving us:
...
$$ \widehat{Extraversion}=0.53-0.99 \times Male $$ {#eq-ExtRegFem}
...
@eq-ExtRegFem is also typically how we will arrange our equation when we’re running a regression. Usually, regression results are displayed in a regression table (as discussed in the prior chapter), so rather than seeing results written out in the form of @eq-ExtRegFem, it would be more typical to find a table listing -0.99 as the coefficient estimate for `male` (or `gender`) and 0.53 as the coefficient estimate for the `intercept` (or the `constant`). @tbl-reg-extraversion-gender shows this more typical display of results, although note that the male coefficient is now shown as the more accurate -0.98 which was altered in the prior discussion to avoid mismatched results due to rounding error.
...
: Results for a regression with extraversion as the dependent variable {#tbl-reg-extraversion-gender}

#### Prediction with more than two categories {#sec-prediction-with-multiple-categories}

To see how our analysis changes when examining a qualitative variable with more than two categories, let's examine a variable recording the response to the question "What hand do you use to write with?" There are three response options: right, left, and both. We interpret these as indicating that someone is right-handed, left-handed, or ambidextrous (using both). To first get an overall sense of how this variable is related to extraversion, we can examine a set of box plots (@fig-boxplot-extraversion-hand). It appears that the median is exactly the same (0) for all three subsamples. This is perhaps unsurprising: we would probably not expect hand dominance to have much relationship to extraversion.

![Distribution of extraversion trait by hand one writes with.](Images/qual-associations/boxplot-extraversion-hand.png){#fig-boxplot-extraversion-hand width="400"}

Since we are focusing on use of regression in this section, let's also consider the mean for each subsample, since the most popular type of regression (ordinary least squares) corresponds directly to the mean (not the median). The mean of the extraversion variable is 0.15 for right-handed respondents, -0.09 for left-handed respondents, and -0.45 for respondents who write with both hands.

If we had to make a guess about the level of extraversion of someone, and all we knew about that person was which hand they wrote with, it would be reasonable to guess the mean or median value for the subsample writing with the same hand as them. Here, we focus on the mean. From what we already saw in @eq-ExtReg, it is relatively straightforward to extend this format to incorporate a third category:

$$ \widehat{Extraversion}=0.15 \times Right-0.09 \times Left-0.45 \times Both$$ {#eq-ExtHand}

For someone who writes with their right hand, we would plug in *Right* = 1, *Left* = 0, and *Both* = 0:

$$ \widehat{Extraversion}=0.15 \times (1) -0.09 \times (0) -0.45 \times (0)=0.15 $$

If someone writes with both hands, we would use *Right* = 0, *Left* = 0, and *Both* = 1:

$$ \widehat{Extraversion}=0.15 \times (0) -0.09 \times (0) -0.45 \times (1)= -0.45$$

We can also return to the format of @eq-ExtRegFem but modify it to include the other category. We start from one category as our baseline. Here, we choose right-handed respondents as our baseline, so we start by writing 0.15, to reflect their average. Then, we must create a coefficient for *Left* indicating the difference in the means of left-handed and right-handed respondents (-0.09-0.15=-0.24). Finally, we create a coefficient for *Both* by subtracting the right-handed mean from the ambidextrous mean (-0.45-0.15=0.60). This type of format is how we will typically write our equation if we are doing a regression:

$$ \widehat{Extraversion}=0.15-0.24 \times Left-0.60 \times Both$$ {#eq-ExtRegHand}

Now, because there are three possible values for the main hand dominance variable (right, left, and both), knowing the value of one of our binary indicators *Left* doesn’t necessarily allow us to conclude what the value of *Right* is. If *Left* = 0, the individual could be either right-handed or ambidextrous. So we have to include a second binary variable. In this case, we chose to include the variable *Both*. If we know the values of *Left* and *Both*, we can always figure out the value of *Right* by process of elimination.

For an ambidextrousperson, we plug in *Left* = 0, and *Both* = 1:

$$ \widehat{Extraversion}=0.15-0.24 \times (0)-0.60 \times (1)=-0.45 $$

When considering a right-handed person, we use *Left* = 0, and *Both* = 0:

$$ \widehat{Extraversion}=0.15-0.24 \times (0)-0.60 \times (0)= 0.15 $$

@eq-ExtHand and @eq-ExtRegHand indicate equivalent predictions for extraversion based on hand dominance; they just offer this information in two different formats. @eq-ExtRegHand might be a bit trickier to understand for now, but it is again the typical form we use with regression.

Notice that we can talk about hand dominance either as one qualitative variable with three possible values (right, left, or both), or we can talk about it as a series of three binary (indicator) variables (*Right*, *Left*, and *Both*) that can each take on a value of either 0 or 1. This can make things a bit confusing, but the important thing to remember is that when we have a qualitative variable with more than two categories, we’ll need to break out the categories into a set of dummy variables for purposes of representing the qualitative variable in an equation.

However, as @eq-ExtRegFem and @eq-ExtRegHand illustrate, we don’t necessarily need a dummy variable for every single category. Specifically, whenever we want to create an equation with a qualitative independent variable in a format like @eq-ExtRegFem or @eq-ExtRegHand, the number of dummy variables should be equal to the number of categories minus one. Since our hand dominance variable can take on three possible values in this example, we included two independent variables in @eq-ExtRegHand. No dummy variable is included for right-handed people, so we call right-handed the **baseline category**, also known as the **reference group** or **omitted category**. Remember, the first number in @eq-ExtRegHand is 0.15, which represents our guess for right-handed people—the baseline category. If we instead had a qualitative variable with five categories, we would include four dummy variables in our equation.
...
1.  What are the basic tools (statistics, tables, and/or graphs) used to describe a relationship between two qualitative variables?

2.  What are the basic tools (statistics, tables, and/or graphs) used to describe a relationship between one qualitative variable and one quantitative variable?

3.  Calculate percentages by row for the following table below. Then, describe whether and how the two variables are related.
    ...
    +------------------+---------------+---------------+---------------+
    | Organization A   | 72            | 28            | 100           |
    |                  |               |               |               |
    |                  | \_\_\_\_\_\_% | \_\_\_\_\_\_% | 100%          |
    +------------------+---------------+---------------+---------------+
    | Organization B   | 166           | 34            | 200           |
    |                  |               |               |               |
    |                  | \_\_\_\_\_\_% | \_\_\_\_\_\_% | 100%          |
    +------------------+---------------+---------------+---------------+
    | **Total**        | 238           | 62            | 300           |
...
4.  Describe the relationship between career ambition and employee type (civilian or uniformed) in the chart below.

    ![](Images/qual-associations/civilian-uniformed.png){width="400"}

5.  Someone collects a sample of bachelor's students and measures each student's age and area of study. Suppose the average age is 21.1 among engineering students, 19.8 among business students, and 20.2 among fine arts students, and 20.7 among humanities students. Write out an equation representing these four means, using business students as the omitted category.

6.  In the regression table below, the dependent variable is GDP per capita measured in thousands of US dollars. Find the mean of country per capita GDP for the seven regions of the world (omitted category is East Asia & Pacific).[^qual-associations-7] Then, describe in words the relationship between income and region.

    +----------------------------+----------------+----------------+
    |                            | Coef.          | Std. err.      |
    +============================+================+================+
    | Europe & Central Asia      | 14.0           | 4.5            |
    +----------------------------+----------------+----------------+
    | Latin America & Caribbean  | -2.2           | 4.9            |
    +----------------------------+----------------+----------------+
    | Middle East & North Africa | -1.5           | 5.8            |
    +----------------------------+----------------+----------------+
    | North America              | 53.9           | 12.6           |
    +----------------------------+----------------+----------------+
    | South Asia                 | -13.5          | 8.2            |
    +----------------------------+----------------+----------------+
    | Sub-Saharan Africa         | -14.2          | 4.7            |
    +----------------------------+----------------+----------------+
    | (intercept)                | 16.5           | 3.5            |
    +----------------------------+----------------+----------------+
    |                            |                |                |
