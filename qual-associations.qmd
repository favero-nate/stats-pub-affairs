...
\[***Working draft notes for instructors**: I ended up deciding to mostly keep the examples for now, but some have been revised for clarity and consistency.*\]
...
Although bar charts can display means, we do not generally recommend them for this purpose. Box plots should be used instead since they provide more information than bar charts without taking up more space. For example, a box plot of the mouse-movement data is shown in @fig-boxplotmousemovement. You can see that @fig-boxplotmousemovement reveals more about the distribution of movement times than does @fig-barchartmeantime. However, quantitative variables that take on only a few discrete values are not very suitable for box plots. For such variables, it is better to stick with displaying means in a bar chart (or something similar like @fig-gender-compare-zoomed-out).
...
#### Prediction with more than two categories for gender {#sec-prediction-with-multiple-categories}
