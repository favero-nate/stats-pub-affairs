...
Note that these formulas are just an approximation for a 95% confidence interval; the formulas for precise intervals are shown in @sec-confidence-interval-for-a-regression-slope-coefficient. In this approximation, all we are doing is multiplying the standard error by two to get the margin of error.[^stat-inference-5] From our initial point estimate of the slope, we can then add or subtract the margin of error to identify a full range of plausible values.
