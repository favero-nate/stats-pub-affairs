...
Note that these formulas are just an approximation for a 95% confidence interval; the formulas for precise intervals are shown in @sec-confidence-interval-for-a-regression-slope-coefficient. Multiplying the standard error by two yields the approximate margin of error.[^stat-inference-4] From our initial point estimate of the slope, we can then add or subtract the margin of error to identify a full range of plausible values.

[^stat-inference-4]: Two is the approximate value by which the standard error should be multiplied, but a more exact value can be found using the t distribution, as explained in @sec-calculating-confidence-intervals.
...
I realize this interpretation is a bit indirect; it is difficult to provide a technically-accurate and meaningful interpretation, despite the fact that confidence intervals have demonstrated great practical value to researchers and analysts.[^stat-inference-5] What makes interpretation difficult is the fact that the "% confidence" in a "95% confidence interval" refers to the accuracy of the *process* of creating a confidence interval—not the probability that a specific confidence interval we encounter will contain the true population parameter. If this distinction seems confusing, it is!

[^stat-inference-5]: Stephens, M. (2023). The Bayesian lens and Bayesian blinkers. *Philosophical Transactions of the Royal Society A*, *381*(2247), 20220144.
...
Fortunately, even if you miss the precise details, you will still probably get something useful out of confidence intervals.[^stat-inference-6] Nonetheless, let's try to set the record straight.

[^stat-inference-6]: Anderson, A. A. (2019). Assessing statistical results: magnitude, precision, and model uncertainty. *The American Statistician*, *73*(sup1), 118-121.

An analogy may help. Suppose you are interacting with a chatbot that is truthful 95% of the time and lies the other 5%.[^stat-inference-7] For each statement, will you always conclude it has a 95% chance of being true? Not necessarily. If the chatbot discusses a topic you already know a lot about, you will probably be able to pick out the lies from the true statements with fairly high confidence. Some things the bot says will be things you know to be true, so you can be nearly 100% sure they are true. Other statements will be things you're quite sure are wrong, so you will conclude that the probability they are true is close to 0%. If you wanted to be very systematic, you could even use the mathematical formula known as Bayes' theorem[^stat-inference-8] to combine your prior knowledge of a statement's probability of being true with the fact that a 95%-accurate bot claimed the statement was true, allowing you to precisely quantify how confident you should be about the statement's truth in the end. 

[^stat-inference-7]: This example is adapted from Behar, R., Grima, P., & Marco-Almagro, L. (2013). Twenty-five analogies for explaining statistical concepts. *The American Statistician*, *67*(1), 44-48.

[^stat-inference-8]: See <https://onlinestatbook.com/2/glossary/bayes.html> or <https://onlinestatbook.com/2/probability/bayes_demo.html>.
...
In the same way, it turns out that *absent any other information,* a 95% confidence interval is often a good approximation for a range of values that contains the population parameter with 95% probability.[^stat-inference-9] Thus, I think it is quite reasonable that many of us, when we see a mean estimate with a 95% confidence interval ranging from A to B, assume there is a 95% chance the population mean does indeed lie between A and B. But technically, that is not a direct interpretation of the confidence interval; instead, this statement about plausible values of the population mean is a subjective conclusion that I can draw based on the confidence interval. Another person might see the same confidence interval and reasonably decide—drawing on their own prior knowledge of the topic—that the confidence interval contains values that are highly implausible, and thus they would reach a different conclusion from me about how likely the interval is to contain the true population mean. 

[^stat-inference-9]: Kass, R. E. (2011). Statistical inference: The big picture. *Statistical science: a review journal of the Institute of Mathematical Statistics*, *26*(1), 1.
