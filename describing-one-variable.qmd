...
\[***Working draft notes for instructors**: This is an early draft of this chapter, and while I expect the basic structure to remain, I am aware that the current examples (including the case study) are underdeveloped. They will be updated shortly (as of June 23, 2026). The section on transformations needs to be entirely rewritten, and I am also considering moving it to another chapter.*\]
...
Let’s revisit the initial example of a box plot we saw in the prior chapter, shown here as a standalone graph in @fig-box-female-employees. Given the long whisker on the left side of the graph, we can say this distribution appears to be left-skewed. Notice that in addition to the box and whiskers, two dots are shown on the left side of the graph. These indicate outliers, which are often ignored for purposes of defining the minimum and maximum values when drawing the whiskers. Dots are typically (though not always) used to indicate the presence of any outliers beyond the range of the whiskers, as in this example. Various algorithms can be used to determine what exactly counts as an outlier, and most software will use one of these algorithms by default when creating a box plot.
...
As one final example of a box plot, let’s examine the individual donations variable from this chapter’s case study. @fig-box-donations contains a box plot for this variable, and unlike the other box plots we’ve seen so far, this one has been drawn vertically. As we noted in the prior section, this variable is right-skewed, which we now see in the form of the long whisker at the top of the graph, since it is now the top of the graph that shows the highest values. There are dots depicting outliers, but they are quite dense, giving the appearance of one thick line rather than several distinct points. Thus, the use of dots to depict outliers is perhaps less effective in this graph; if using this graph to present results to others, we might want to consider omitting the outlier dots altogether (or displaying them in a different way, such as using open circles and perhaps adding some “jitter,” as in the strip plot shown in the prior chapter).
