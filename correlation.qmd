...
\[***Working draft notes for instructors**: I am hoping to replace the linear regression example so that it comes from the chapter case study dataset. If you were looking at this in June, note that I have now (as of July 1) moved the section on variable transformations to the middle of this chapter (it was previously in Ch. 3).*\]
...
## Chapter Case Study: World Bank Development Indicators {.unnumbered}

Data examples throughout this chapter come from the 2018 World Bank Development Indicators dataset.[^correlation-1] This dataset describes various countries around the world. For an initial example, we consider two variables: a common measure of country wealth (logged per capita GDP) and an estimate of perceived political stability that the World Bank generates by combining several data sources. Political stability is reported in standardized units (z-scores), which are explained later in this chapter. For now, it is enough to know that 0 indicates average political stability while negative values indicate lower-than-average stability and positive values indicate greater-than-average stability. A later section will also explain what is meant by *logged* per capita GDP.

Visually, we can depict the relationship between two quantitative variables using a scatter plot, as shown in @fig-stability-gdp. The basic pattern from this plot is that wealthier countries (represented by dots further to the right) tend to be more politically stable (indicated by the dots appearing closer to the top of the graph). Notice the general absence of dots in the bottom-right corner: this means that few countries are both very wealthy and politically unstable.
...
@tbl-corr-benchmarks provides example language we might use to describe the strength of a relationship based on a correlation coefficient. For example, a correlation of -0.42 can be considered "moderate" while a correlation of 0.23 can be called "weak." However, the labels from the table should only be considered rough guidelines. As with many other aspects of statistics, the context of the data should really inform our interpretation. For example, many areas of social science focus on aspects of human behavior that are difficult to explain, such as how attitudes are linked to many behaviors.[^correlation-3] In such contexts, it may be somewhat rare to find a correlation outside the narrow range of -0.2 to 0.2. Someone doing analysis in one of these contexts might therefore reasonably describe a correlation of 0.3 as "strong," given that the correlation is indeed strong *relative* to most other correlations in that context. This is a bit like how the way we use the term "big" changes based on context (e.g., a big insect is still a small creature in the forest).

[^correlation-3]: Bosco, F. A., Aguinis, H., Singh, K., Field, J. G., & Pierce, C. A. (2015). Correlational effect size benchmarks. *Journal of applied psychology*, *100*(2), 431.

+-------+--------------+--------------+--------------+------------------+------------+------------+------------+
|       | Strong       | Moderate     | Weak         | Mostly Unrelated | Weak       | Moderate   | Strong     |
+=======+==============+==============+==============+==================+============+============+============+
| **r** | -1.0 to -0.6 | -0.5 to -0.3 | -0.2 to -0.1 | Rounding to 0.0  | 0.1 to 0.2 | 0.3 to 0.5 | 0.6 to 1.0 |
+-------+--------------+--------------+--------------+------------------+------------+------------+------------+

: Example language to interpret the strength of a correlation, based roughly on Cohen's effect size benchmarks {#tbl-corr-benchmarks}

If you're interested in learning more about exactly how the values of a correlation are calculated, further details are provided in Appendix I.

## Transforming Variables[^correlation-4] {#sec-transforming-variables}

[^correlation-4]: The initial part of this section is adapted from David M. Lane. “Linear Transformations.” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/introduction/linear_transforms.html>. There is also material adapted from David M. Lane. “Standard Normal Distribution.” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/normal_distribution/standard_normal.html>.

Especially when describing relationships between variable, it is often necessary to transform data from one measurement scale to another. What do we mean by measurement scales? Some of the simplest examples come from ordinary measures of the physical world. For example, you might want to convert height measured in feet to height measured in inches. @tbl-heightconversion shows the heights of four people measured in both feet and inches. To transform feet to inches, you simply multiply by 12. Similarly, to transform inches to feet, you divide by 12.
...
As a simple application, suppose you want the $z$ score for a student who got a 26 on an exam, where the full set of exam-takers have a distribution with a mean of 50 and a standard deviation of 10. Applying the formula, we obtain:

$$ z = (26 - 50)/10 = -2.4 $$

The value -2.4 indicates that the student scored 2.4 standard deviations below the mean.
...
### Log Transformations[^correlation-5]

[^correlation-5]: This subsection is adapted from David M. Lane. “Log Transformations.” *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/transformations/log.html>
...
In fact, we have already been making use of the log transformation when looking at country wealth in this chapter, although I did not previously draw attention to the transformation. "*Logged* per capita GDP" refers to a log transformed version of per capita GDP. Without this log transformation, the per capita GDP measure would be skewed right with large outliers for very wealthy countries.

Let's turn briefly to the natural sciences for a particularly stark example of how log transformations are useful. @fig-scatterplotbrainweight shows how a log transformation can make patterns more visible. Both graphs plot the brain weight of animals as a function of their body weight. The raw weights are shown in the upper panel; the log-transformed weights are plotted in the lower panel.
...
[^correlation-7]: David M. Lane. "Introduction to Linear Regression." *Online Statistics Education: A Multimedia Course of Study*. <https://onlinestatbook.com/2/regression/intro.html>

## Quick Guide to Interpreting Regression Results {#sec-quick-guide-to-interpreting-regression-results}

Many social science papers report their main results in the form of a regression table. It's fairly easy to get started interpreting these results using the three S's:[^correlation-8]

[^correlation-8]: Wheelan, C. (2010.) *Introduction to Public Policy*. New York: W. W. Norton & Company.

::: {.callout-tip icon="false"}
## The Three S's (of Regression Results) {.unnumbered}

-   **Significance**: Is the relationship between the two variables strong enough (relative to the precision of the estimate) to be considered statistically reliable?[^correlation-9] To assess this, check the p-value. For now, you can use the following rule-of-thumb:
...
-   **Size**: How big is the (predictive) effect? This S is often the most difficult to make sense of, and sometimes you may not have enough information to meaningfully evaluate it (e.g., if the units of measurement for a variable are not clearly explained).
    ...
    -   For linear models: When the independent variable increases by one unit, we adjust our prediction for the dependent variable by $\hat{\beta_i}$ (where $\hat{\beta_i}$ represents the value of the coefficient estimate)[^correlation-10]
...
[^correlation-9]: In other words, can we conclude it is signal rather than noise? See: Fricker Jr, R. D., Burke, K., Han, X., & Woodall, W. H. (2019). Assessing the statistical analyses used in basic and applied social psychology after their p-value ban. *The American Statistician*, *73*(sup1), 374-384.

[^correlation-10]: Why $\hat{\beta_i}$ and not simply $\beta_i$? We use the hat symbol ($\hat{~}$) to indicate an estimate or prediction. So by including the hat, we are implying that our coefficient is an estimate. We'll learn more about estimation in @sec-stat-inference.
...
## @sec-correlation Appendix I: How Correlation is Calculated {.unnumbered}

There are many ways to write the formula for correlation. As the second appendix from the prior chapter explained, we make a distinction between statistics for a "sample" versus a "population" when writing equations for certain statistics (e.g., variance). It is more common to have data from a sample, but we will focus in this appendix on a formula describing the population since it is a bit simpler. Additional formulas (including for samples) are presented in the next appendix. Note that we use the Greek letter $\rho$ (rather than $r$) to represent the correlation when we are describing a population.
...
The Data Excerpt below shows one way to compute a correlation manually in a spreadsheet. Only 9 observations are shown in the table, but the full dataset consists of 204 observations. The means at the bottom of the table are computed from the full dataset.

In the Data Excerpt, we see that the variables `stability` and `urban` have been transformed to z-scores (columns labeled `z_stability` and `z_urban`).[^correlation-12] In the final column, the values of the z-scores for the two variables are multiplied together. The average of this final column shown at the bottom of the table indicates the correlation between `stability` and `urban`: 0.36. Thus, there is a weak but noticeable positive association between the two variables.

```         
Data Excerpt
+------+-------------+-----------+--------+--------+-------+----------+
| id   | country     | stability | urban  | z_stab | z_urb | z_stab   |
|      |             |           |        |        |       |  × z_urb |
+======+=============+===========+========+========+=======+==========+
...
+------+-------------+-----------+--------+--------+-------+----------+
| 4    | American    | 1.22      | 87.15  | 1.25   | 1.13  | 1.41     |
|      |  Samoa      |           |        |        |       |          |
+------+-------------+-----------+--------+--------+-------+----------+
...
+------+-------------+-----------+--------+--------+-------+----------+
| 7    | Antigua     | 0.84      | 24.60  | 0.87   | -1.50 | -1.31    |
|      |  and        |           |        |        |       |          |
|      |  Barbuda    |           |        |        |       |          |
+------+-------------+-----------+--------+--------+-------+----------+
...
This same data is depicted in @fig-scatterplot-quadrants, with `id` numbers used to plot the 9 observations shown in the Data Excerpt; the remainder of the observations are plotted as circles.
...
[^correlation-12]: Note that standardizing was accomplished using the population variance formula, in order to match the calculation of the mean for the final column, which divides by N rather than n - 1.
...
## @sec-correlation Appendix II: Formulas for Correlation, Covariance, and Regression Coefficients {.unnumbered}

If we are working with a sample, we can rewrite @eq-pop-corr (from Appendix I) as:
