...
@tbl-likes-by-length provides a comparison of the number of "likes" for short versus long posts.
...
### Graphs for data with many different numeric values {#sec-quantitative-variables}
...
For another example, we can use data on the proportion of government employees who are female in different countries around the world. @fig-hist-females_publ visualizes this data, which displays a certain asymmetry: the bars on the right half of the graph tend to be taller than those on the left. The x-axis shows the range of proportions seen accross the countries. We can convert a proportion to a percentage by shifting the decimal point two places to the right. So a country where the proportion of government employees who are female is .2 would have 20% female employees. The short bars in the graph indicate that relatively few countries have proportions in the approximate range of .1 to .45; in other words, for a small number of countries, we see that 10-45% of the government workforce is female. Most countries have a government workforce that is more like 45-75% female (the tall bars on the right side of the graph). Thus, it seems that in a majority of the 44 countries included in this dataset, the government employs more females than males.
...
A survey of nonprofits asked each respondent to estimate the percentage of individual donations to their organization in 2019 that were smaller than \$250.[^getting-started-1] Look at how different the right-most bars of the histogram look, depending on whether we use 10 bins or 11 bins (a setting we can change in the software creating the histogram):
...
[^getting-started-1]: Year 1 of the Nonprofit Trends Longitudinal Survey Public Use Data Files. <https://doi.org/10.7910/DVN/T4OT1J>
...
Public health officials advise that at least 95% of the community should be vaccinated against measles in order to maintain herd immunity.[^getting-started-3] @fig-vaccines-zoomed-out shows estimates for the rate of measles-mumps-rubella (MMR) vaccination in the U.S. among children starting primary school (solid line).[^getting-started-4] The y-axis is drawn to start at 0, and the target rate of 95% is depicted as a dashed line. The solid line looks almost flat and is always close to the dashed line. It is hard to say much more based on this graph, except that the actual vaccination rate appears to fall a bit shy of the 95% target in recent years.
...
[^getting-started-3]: Pandey, A., & Galvani, A. P. (2023). Exacerbation of measles mortality by vaccine hesitancy worldwide. *The Lancet Global Health*, *11*(4), e478-e479. <https://doi.org/10.1016/S2214-109X(23)00063-3>

[^getting-started-4]: [Data](https://data.cdc.gov/Vaccinations/Vaccination-Coverage-and-Exemptions-among-Kinderga/ijqb-a7ye) from the U.S. Centers for Disease Control and Prevention.
